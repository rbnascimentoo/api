...
public class UserServiceTest {
	...
	public void updateUsers() {
		...
		User userCreation = User.builder().name("User Test").login("login").password(passwordHash).role(Role.ADMINISTRATOR).build();
		User userCreated = User.builder().id(1L).name("User Test").login("login").password(passwordHash).role(Role.ADMINISTRATOR).build();
		UserDTO userDto = UserDTO.builder().name("User Test").login("login").password("password").role(Role.ADMINISTRATOR).build();
		
		User userUpdate = User.builder().id(1L).name("User updated").login("login").password(passwordHash).role(Role.ADMINISTRATOR).build();
		UserDTO userUpdateDto = UserDTO.builder().id(1L).name("User updated").login("login").password("password").role(Role.ADMINISTRATOR).build();
	...
	public void findUserByLogin() {
		...
		Optional<User> user = Optional.of(User.builder().id(1L).name("User Test").login(login).password(passwordHash).build());
